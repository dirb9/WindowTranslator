...
using WindowTranslator.Plugin.OneOcrBrtnsCntrstPlugin.Properties;
using WinRT;
using static WindowTranslator.LanguageUtility;
using static WindowTranslator.OcrUtility;
using static WindowTranslator.Plugin.OneOcrBrtnsCntrstPlugin.NativeMethods;

namespace WindowTranslator.Plugin.OneOcrBrtnsCntrstPlugin;
...
public sealed class OneOcrBrtnsCntrst : IOcrModule, IDisposable
{
    ...
    static OneOcrBrtnsCntrst()
    {
        var context = AssemblyLoadContext.GetLoadContext(typeof(OneOcrBrtnsCntrst).Assembly) ?? throw new InvalidOperationException();
    ...
    public OneOcrBrtnsCntrst(ILogger<OneOcrBrtnsCntrst> logger, IOptionsSnapshot<LanguageOptions> langOptions, IOptionsSnapshot<BasicOcrParam> ocrParam)
    {
        this.logger = logger;
        this.fastText = new FastTextDetector();
        this.fastText.LoadDefaultModel();
        this.source = langOptions.Value.Source;
        this.targets = new HashSet<string> { langOptions.Value.Target[..2] };
        if (this.targets.Overlaps(new[] { "ja", "zh" }) && this.source[..2] is not "ja" and not "zh")
        {
            // 日本語と中国語の判定は難しいので、お互いの翻訳でないときは、どちらも翻訳対象外となる翻訳先言語とする
            this.targets.UnionWith(new[] { "ja", "zh" });
        }
        // 閾値パラメータの設定
    ...
    async ValueTask<SoftwareBitmap> AdjustBrightnessContrast(SoftwareBitmap src, int brightness, double percentileContrast)
    {
        ...
        double realContrast = (percentileContrast <= -100.0) ?
            0.01 :
            (percentileContrast + 100.0) / 100.0; // コントラストの計算に使用する値
        ...
        unsafe
        {
            ...
            for (uint i = 0; i < capacity; i += 4)
            {
                ...
                r = (r - 128) * realContrast + 128 + brightness;
                g = (g - 128) * realContrast + 128 + brightness;
                b = (b - 128) * realContrast + 128 + brightness;
    ...
    public async ValueTask<IEnumerable<TextRect>> RecognizeAsync(SoftwareBitmap bitmap)
    {
        ...
        var workingBitmap = await AdjustBrightnessContrast(bitmap, -20, 90);
    ...
    private bool CanMerge(TextRectMerger temp, TextRect rect, double xThreshold, double yThreshold)
    {
        ...
        if (IsSpaceLang(this.source) ?
            (WordCount(temp.Text) <= 1) :
            (temp.Text.Length <= 8))
        {
            return false;
        }
    ...
    private class TextRectMerger
    {
        ...
        public TextRectMerger(TextRect rect)
        {
            ...
            this.rects = new List<TextRect> { rect };
        }
